...
@section('content')
    ...
    <form action="{{ route('admin.videos.update', $video->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-lg p-8">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">{{ old('description', $video->description) }}</textarea>
        </div>
        ...
        <div class="mb-4">
            ...
            @if ($video->pdf_path)
                <p class="mt-2 text-sm">
                    <a href="{{ asset('storage/' . $video->pdf_path) }}" target="_blank"
                       class="text-indigo-600 hover:underline">📄 View current PDF</a>
                </p>
            @endif

            <p class="text-xs text-gray-500 mt-1">Upload a new PDF to replace the existing one (optional).</p>
        </div>

        <div class="mb-4">
            <label for="order" class="block text-sm font-medium text-gray-700">Order</label>
            <input type="number" name="order" id="order" value="{{ old('order', $video->order) }}"
                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                   required>
        </div>
